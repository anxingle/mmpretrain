...
model = dict(
    ...
    head=dict(
        type='LinearClsHead',
        num_classes=2,  # 裂纹(crack) vs 正常(normal)
        in_channels=1280,
        loss=dict(
            type='CrossEntropyLoss',
            # 类别权重：处理类别不平衡
            # normal:crack = 3467:2037 ≈ 1.7:1
            # 使用反比例权重：crack权重=3467/2037≈1.7, normal权重=1.0
            class_weight=[1.5, 1.0],  # [crack, normal]
            loss_weight=1.0),
        topk=(1,),  # 二分类只看top1准确率
    ))
...
train_pipeline = [
    ...
    dict(
        type='ColorJitter',
        brightness=0.25, # 亮度扰动范围 75%~125% 之间随机调整
        contrast=0.25, # 对比度
        saturation=0.189, # 饱和度
        hue=[0.00001, 0.0159], # 色调偏移范围
        backend='pillow',
    ),
    # 2. 轻度高斯模糊（模拟拍摄时的轻微失焦，但不能太强以免影响裂纹纹理）
    dict(
        type='GaussianBlur',
        magnitude_range=(0.3, 0.7),  # 较轻的模糊程度，保护裂纹纹理
        magnitude_std='inf',
        prob=0.6  # 60%概率应用模糊
    ),
...
train_dataloader = dict(
    ...
    dataset=dict(
        type="CustomDataset",
        data_root='/home/an/mmpretrain/works/datasets/crack_VS_normal_0812',
...
test_dataloader = dict(
    ...
    dataset=dict(
        type="CustomDataset",
        data_root='/home/an/mmpretrain/works/datasets/tests_3multi/crack_VS_normal_0812_test',
        pipeline=test_pipeline),
    sampler=dict(type='DefaultSampler', shuffle=False),
)
...
custom_hooks = [
    dict(
        ...
        enable=True,
        interval=5,  # 每5次迭代可视化一次
        show=False,
        draw_gt=True,
        draw_pred=True,
        out_dir='./logs/vis_crack_normal',  # 可视化输出目录
    ),
]
