...
model = dict(
    ...
    head=dict(
        type='LinearClsHead',
        num_classes=2,  # 点刺(swift) vs 正常(normal)
        in_channels=1280,
        loss=dict(
...
train_pipeline = [
    ...
    dict(
        type='ColorJitter',
        brightness=0.15, # 亮度扰动范围 80%~120% 之间随机调整
        contrast=0.15, # 对比度
        saturation=0.159, # 饱和度
        hue=[0.00001, 0.0099], # 色调偏移范围
        backend='pillow',
    ),
    # 2. 轻度高斯模糊（模拟拍摄时的轻微失焦，但不能太强以免影响点刺细节）
    dict(
        type='GaussianBlur',
        magnitude_range=(0.2, 0.45),  # 很轻的模糊程度，保护点刺细节
        magnitude_std='inf',
        prob=0.45  # 45%概率应用模糊
    ),
