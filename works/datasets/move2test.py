...
if __name__ == "__main__":
	source_directory = Path("/data-ssd/coating6000_color/white")
	target_directory = Path("/data-ssd/coating6000_color_test/white")
	move_num = 70
	...
	# all_train_images = [f.name for _dir in Path("/data-ssd/coating6000_color").iterdir() if _dir.is_dir() and _dir.name != "normal"  for f in _dir.iterdir() if f.is_file() and f.suffix in [".png", ".jpg"]]
	# all_test_images = [f.name for _dir in Path("/data-ssd/coating6000_color_test").iterdir() if _dir.is_dir() and _dir.name != "normal"  for f in _dir.iterdir() if f.is_file() and f.suffix in [".png", ".jpg"]]

	# all_test_images_normal = [f.name for f in Path("/data-ssd/coating6000_color_test/normal").iterdir() if f.is_file() and f.suffix in [".png", ".jpg"]]
