...
model = dict(
    ...
    head=dict(
        ...
        loss=dict(
            ...
            # 类别权重：处理类别不平衡
            # ('gray', 'white', 'yellow')数量为(22, 668, 4844)
            class_weight=[9.1, 1.66, 0.62],  # [gray, white, yellow]
            loss_weight=1.0),
        topk=(1,),  # 二分类只看top1准确率
    ))
...
train_pipeline = [
    ...
    dict(
        type='ColorJitter',
        brightness=0.02, # 亮度扰动范围 80%~120% 之间随机调整
        contrast=0.02, # 对比度
        saturation=0.01, # 饱和度
        hue=[0.00001, 0.0001], # 色调偏移范围
        backend='pillow',
    ),
    # 2. 轻度高斯模糊（模拟拍摄时的轻微失焦）
    dict(
        type='GaussianBlur',
        magnitude_range=(0.1, 0.4),  # 较轻的模糊程度
        magnitude_std='inf',
        prob=0.4  # 50%概率应用模糊
    ),
