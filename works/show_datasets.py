...
train_pipeline = [
    ...
    dict(
        ...
        selective_grayscale=True,  # 自定义扩展：部分保留颜色
        keep_color_threshold=0.10,  # 30%区域保留颜色
        keep_channels=True, # 保持3通道格式
        color_format='bgr' # 需要确定图像格式 
    ),
    # XXX: 随机高斯模糊
    dict(
        type='GaussianBlur',
        magnitude_range=(0.15, 0.6),
        magnitude_std='inf',
    ...
    dict(
        ...
        save_dir='./randaug_visualization',  # 图像保存目录
        sample_rate=0.9),  # 采样率(10%的图像会被保存)
...
train_dataloader = dict(
    ...
    dataset=dict(
        type="CustomDataset",
        data_root='/home/an/mmpretrain/works/fat_thin_normal_datasets',
        split='train',
...
val_dataloader = dict(
    ...
    dataset=dict(
        type="CustomDataset",
        data_root='/home/an/mmpretrain/works/test_fat_thin_normal_datasets',
        split='val',
